...
app.post('/api/broadcast/playlists', async (req, reply) => {
  ...
  const contextFormatSettings =
    placement.format?.settings && typeof placement.format.settings === 'object'
      ? (placement.format.settings as Record<string, unknown>)
      : {};
  const contextFormatRecord = (key: string) => {
    const value = contextFormatSettings[key];
    ...
    for (const videoId of body.youtubeVideoIds) {
      ...
      await addBroadcastYoutubeContextItem(
        ...
        },
        {
          ...
          newsFallback,
          pauseDuringAva: true,
          formatSystemKey: placement.format?.system_key ?? null,
          contextLayoutVariant:
            typeof contextFormatSettings.youtubeContextLayoutVariant === 'string'
              ? contextFormatSettings.youtubeContextLayoutVariant
              : null,
          formatName: placement.format?.name ?? null,
          formatConcept:
            typeof contextFormatSettings.formatConcept === 'string' ? contextFormatSettings.formatConcept : null,
          moderationIntent:
            typeof contextFormatSettings.moderationIntent === 'string' ? contextFormatSettings.moderationIntent : null,
          accentColor: placement.format?.color ?? null,
          avaRole: contextFormatRecord('avaRole'),
          miaRole: contextFormatRecord('miaRole'),
          samRole: contextFormatRecord('samRole'),
          coHostRole: contextFormatRecord('coHostRole'),
          coHostRoles: contextFormatRecord('coHostRoles'),
          hostChoreography: contextFormatRecord('hostChoreography'),
          editorialSafety: contextFormatRecord('editorialSafety'),
          miaInteractionPrompt:
            typeof contextFormatSettings.miaInteractionPrompt === 'string'
              ? contextFormatSettings.miaInteractionPrompt
              : typeof contextFormatRecord('miaRole').prompt === 'string'
                ? String(contextFormatRecord('miaRole').prompt)
                : null,
          comedyMode: contextFormatSettings.comedyMode === true,
          satireMode: contextFormatSettings.satireMode === true,
          satireLabel: contextFormatSettings.satireLabel === true,
          coHostId: typeof contextFormatSettings.coHostId === 'string' ? contextFormatSettings.coHostId : null,
          coHostIds: Array.isArray(contextFormatSettings.coHostIds)
            ? contextFormatSettings.coHostIds.filter(
                (value): value is string => typeof value === 'string' && Boolean(value.trim()),
              )
            : [],
          hostRoster: Array.isArray(contextFormatSettings.hostRoster)
            ? contextFormatSettings.hostRoster.filter(
                (value): value is string => typeof value === 'string' && Boolean(value.trim()),
              )
            : [],
...
async function youtubeContextOverlayPayload(
  ...
  return {
    ...
    youtubeContext: {
      ...
      coHostRole: rules.coHostRole && typeof rules.coHostRole === 'object' ? rules.coHostRole : {},
      coHostRoles: rules.coHostRoles && typeof rules.coHostRoles === 'object' ? rules.coHostRoles : {},
      hostChoreography:
        rules.hostChoreography && typeof rules.hostChoreography === 'object' ? rules.hostChoreography : {},
      editorialSafety: rules.editorialSafety && typeof rules.editorialSafety === 'object' ? rules.editorialSafety : {},
      comedyMode: rules.comedyMode === true,
      satireMode: rules.satireMode === true,
      satireLabel: rules.satireLabel === true,
      coHostId: typeof rules.coHostId === 'string' ? rules.coHostId : null,
      coHostIds: Array.isArray(rules.coHostIds)
        ? rules.coHostIds.filter((value): value is string => typeof value === 'string' && Boolean(value.trim()))
        : [],
      hostRoster: Array.isArray(rules.hostRoster)
        ? rules.hostRoster.filter((value): value is string => typeof value === 'string' && Boolean(value.trim()))
        : [],
      ...
      coHostIdleVideoUrl: (host as any)?.coHost?.idleVideoUrl ?? null,
      coHostSpeakingVideoUrl: (host as any)?.coHost?.speakingVideoUrl ?? null,
      coHosts: Array.isArray((host as any)?.coHosts) ? (host as any).coHosts : [],
    },
    overlay: baseOverlay,
...
function rendererHtml(dataUrl: string, overlayToken?: string) {
  const style = [
    ...
    '.youtube-context-stage.layout-politik-comedy .youtube-context-avatar,.youtube-context-stage.layout-politik-comedy .youtube-context-copy{border-color:rgba(251,191,36,.86)}.youtube-context-stage.layout-politik-comedy .youtube-context-avatar{background:radial-gradient(circle at 50% 34%,rgba(251,191,36,.16),rgba(16,10,4,.94) 65%)}.youtube-context-stage.layout-politik-comedy .youtube-context-copy{border-left-color:#fbbf24;background:linear-gradient(135deg,rgba(23,14,3,.98),rgba(36,22,5,.95))}.youtube-context-stage.layout-politik-comedy .youtube-context-kicker,.youtube-context-stage.layout-politik-comedy .youtube-context-status{color:#fde68a}.youtube-context-stage.layout-politik-comedy .youtube-context-status:before{background:#fbbf24;box-shadow:0 0 15px #fbbf24}',
    ...
    '.youtube-context-chat-video{position:absolute;right:-18px;bottom:0;width:72%;height:98%;object-fit:contain;object-position:50% 100%;filter:drop-shadow(0 18px 24px rgba(0,0,0,.6));opacity:0;translate:90px 0;transition:opacity .34s ease,translate .52s cubic-bezier(.16,1,.3,1);visibility:hidden}.youtube-context-stage.preparing-chat .youtube-context-chat-video{visibility:visible;opacity:1;translate:0 0}.youtube-context-stage.preparing-chat .youtube-context-ava-video,.youtube-context-stage.preparing-chat .youtube-context-cohost-video{visibility:hidden;opacity:0}',
    '.youtube-context-cohost-video{position:absolute;right:-8px;bottom:0;width:57%;height:98%;object-fit:contain;object-position:50% 100%;filter:drop-shadow(0 18px 24px rgba(0,0,0,.62));opacity:0;translate:70px 0;visibility:hidden;transition:opacity .32s ease,translate .45s cubic-bezier(.16,1,.3,1),scale .35s ease}.youtube-context-stage.layout-politik-comedy .youtube-context-cohost-video{visibility:visible;opacity:.88;translate:0 0}.youtube-context-stage.layout-politik-comedy .youtube-context-ava-video{width:59%;translate:-82px 0}.youtube-context-stage.layout-politik-comedy.cohost-speaking .youtube-context-cohost-video{opacity:1;scale:1.08;filter:drop-shadow(0 18px 30px rgba(251,191,36,.48))}.youtube-context-stage.layout-politik-comedy.cohost-speaking .youtube-context-ava-video{opacity:.54;scale:.92}.youtube-context-stage.layout-politik-comedy.ava-speaking .youtube-context-cohost-video{opacity:.54;scale:.92}',
    '.youtube-context-stage.chat-speaking .youtube-context-ava-video{width:59%;translate:-82px 0;opacity:.42}.youtube-context-stage.chat-speaking .youtube-context-cohost-video{opacity:.22;scale:.9}.youtube-context-stage.chat-speaking .youtube-context-chat-video{visibility:visible;opacity:1;translate:0 0}',
  ...
  const script = [
    ...
    "    'youtubeContext.formatName':data.youtubeContext?.formatName,",
    ...
    '  if(!active||currentDoc?.template!=="youtube-context"){if(youtubeAnalysisHold)youtubeAnalysisHold.remove();youtubeAnalysisHold=null;return}if(!youtubeAnalysisHold){const hold=document.createElement("div"),badge=document.createElement("div"),grid=document.createElement("div");hold.className="youtube-analysis-hold";badge.className="youtube-analysis-badge";grid.className="youtube-analysis-grid";hold.append(badge,grid);youtubeAnalysisHold=hold}const frame=contextVideoFrame(currentDoc),accent=currentDoc?.formatIdentity?.accent||"#22d3ee",focusCoHost=contextCoHostForTurn(contextFocusSnapshot?.turn,contextFocusSnapshot?.host),focusPresenter=focusCoHost?.name||(contextFocusSnapshot?.host?.moderator?.name||"AVA");youtubeAnalysisHold.style.left=frame.x+"px";youtubeAnalysisHold.style.top=frame.y+"px";youtubeAnalysisHold.style.width=frame.width+"px";youtubeAnalysisHold.style.height=frame.height+"px";youtubeAnalysisHold.style.setProperty("--analysis-radius",frame.radius+"px");youtubeAnalysisHold.style.setProperty("--analysis-accent",accent);youtubeAnalysisHold.className="youtube-analysis-hold"+(phase==="returning"?" focus-returning":"");youtubeAnalysisHold.querySelector(".youtube-analysis-badge").textContent=phase==="returning"?"VIDEO STARTET":"VIDEO PAUSIERT · "+focusPresenter.toUpperCase()+" ORDNET EIN";if(youtubeAnalysisHold.parentNode!==root)root.appendChild(youtubeAnalysisHold)',
    '}',
    ...
    'function contextCoHostForTurn(turn,host=lastHostState){const presenters=Array.isArray(host?.coHosts)&&host.coHosts.length?host.coHosts:(host?.coHost?[host.coHost]:[]);return presenters.find(candidate=>candidate?.id===turn?.presenterId)||null}',
    'function isContextCoHostTurn(turn,host=lastHostState){return Boolean(contextCoHostForTurn(turn,host))}',
    ...
    'function contextCoHostVideo(layer){return layer?.querySelector(".youtube-context-cohost-video")||null}',
    ...
    'function setContextCoHostVideo(layer,url,play=false){const video=contextCoHostVideo(layer);if(!video)return;if(url&&video.dataset.src!==url){video.dataset.src=url;video.src=url;video.load()}if(play)video.play().catch(()=>{});else{video.pause();try{video.currentTime=0}catch{}}}',
    'function speakingVideoForTurn(layer,turn,host=lastHostState){return layer===youtubeContextStage?(isContextChatTurn(turn)?contextChatVideo(layer):isContextCoHostTurn(turn,host)?contextCoHostVideo(layer):contextAvaSpeakingVideo(layer)):layer?.querySelector(".ai-host-avatar-video")}',
    'function speakingUrlForTurn(layer,turn,host){const coHost=contextCoHostForTurn(turn,host);if(layer===youtubeContextStage)return isContextChatTurn(turn)?(host.chatModerator?.videoUrl||host.moderator?.chatModeratorVideoUrl):coHost?(coHost.speakingVideoUrl||coHost.avatarVideoUrl):(host.moderator?.speakingVideoUrl||host.moderator?.avatarVideoUrl);return coHost?.avatarVideoUrl||host.moderator?.avatarVideoUrl||null}',
    ...
    '  audio.addEventListener("playing",()=>{playedHostTurns.add(turn.id);pendingHostAudioTurn=null;pendingHostItemId=null;activeHostAudioTurn=turn.id;activeHostItemId=host.broadcastItemId||null;const layer=hostLayerForTurn(turn.id);if(layer)layer.dataset.voicePhase="speaking";layer?.classList.remove("voice-waiting","voice-finished","preparing-chat","entering");layer?.classList.add("speaking",isContextChatTurn(turn)?"chat-speaking":isContextCoHostTurn(turn,host)?"cohost-speaking":"ava-speaking");if(layer===youtubeContextStage){setContextFocusPhase(turn.id,"speaking");if(lastYoutubeContextState)renderYoutubeContext(lastYoutubeContextState,lastHostState)}const video=speakingVideoForTurn(layer,turn,host);if(video){video.pause();try{video.currentTime=0}catch{}video.play().catch(()=>{})}},{once:true});',
