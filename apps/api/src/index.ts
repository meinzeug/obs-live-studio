...
app.post('/api/overlay/audio-duck', async (req, reply) => {
  ...
  const turnDisplayMode = input.turnId
    ? ((
        await query<{ display_mode: 'takeover' | 'inline' }>(`select display_mode from ai_staff_turns where id=$1`, [
          input.turnId,
        ])
      ).rows[0]?.display_mode ?? 'takeover')
    : 'takeover';
  ...
  if (input.action === 'start' && input.itemId && input.turnId) {
    ...
    if (pauseEnabled === true && turnDisplayMode !== 'inline') {
...
function rendererHtml(dataUrl: string, overlayToken?: string) {
  const style = [
    ...
    '@keyframes contextBrandTakeoverIn{from{opacity:.2;clip-path:circle(16% at 25% 48%);filter:saturate(.8) brightness(.28)}to{opacity:1;clip-path:circle(90% at 50% 50%);filter:saturate(1.12) contrast(1.08) brightness(.54)}}@keyframes contextBrandTakeoverOut{from{opacity:1}to{opacity:0;filter:brightness(.25)}}',
    '@keyframes contextTakeoverIn{from{opacity:0;clip-path:inset(0 100% 0 0)}to{opacity:1;clip-path:inset(0)}}@keyframes contextTakeoverOut{from{opacity:1;clip-path:inset(0)}to{opacity:0;clip-path:inset(0 0 0 100%)}}@keyframes contextPresenterRise{from{opacity:0;translate:0 90px;scale:.92}to{opacity:1;translate:0 0;scale:1}}',
  ].join('');
  const script = [
    ...
    '  const takeover=Boolean(doc.template==="youtube-context"&&contextTakeoverTurn&&contextTakeoverPhase!=="idle");studioBrandBackground.className="studio-brand-background "+doc.template+(takeover?" presenter-takeover":"")+(takeover&&contextTakeoverPhase==="returning"?" takeover-returning":"");studioBrandBackground.style.display="";if(studioBrandBackground.parentNode!==root)root.prepend(studioBrandBackground);studioBrandBackground.play().catch(()=>{})',
    '}',
    'function syncStudioBrandTakeover(restart=false){',
    '  if(!studioBrandBackground)return;const active=Boolean(contextTakeoverTurn&&contextTakeoverPhase!=="idle");studioBrandBackground.classList.toggle("presenter-takeover",active);studioBrandBackground.classList.toggle("takeover-returning",active&&contextTakeoverPhase==="returning");if(restart&&active){try{studioBrandBackground.currentTime=0}catch{}studioBrandBackground.play().catch(()=>{})}',
    '}',
    'function beginContextTakeover(turn,host){',
    '  if(!turn?.id)return;const changed=contextTakeoverTurn!==turn.id;if(contextTakeoverExitTimer){clearTimeout(contextTakeoverExitTimer);contextTakeoverExitTimer=null}contextTakeoverTurn=turn.id;contextTakeoverPhase="entering";contextTakeoverSnapshot={turn,host};syncStudioBrandTakeover(changed)',
    '}',
    'function setContextTakeoverPhase(turnId,phase){',
    '  if(contextTakeoverTurn!==turnId)return;contextTakeoverPhase=phase;syncStudioBrandTakeover()',
    '}',
    'function clearContextTakeover(turnId){',
    ...
    '  const takeoverState=contextTakeoverPhase==="entering"?"STUDIO ÜBERNIMMT":contextTakeoverPhase==="returning"?"VIDEO STARTET":"VIDEO PAUSIERT",set=(role,value)=>{const node=youtubeContextStage.querySelector("[data-context-role="+role+"]");if(node)node.textContent=value||""};set("status",onAir?(contextChat?(displayHost?.chatModerator?.name||"MIA")+" LIVE":inlineCommentary?"AVA KOMMENTIERT":"AVA LIVE"):"AVA IM STUDIO");set("kind",kind);set("count",takeover?takeoverState:inlineCommentary?"VIDEO LÄUFT":onAir?"VIDEO PAUSIERT":context.cardCount?String((context.cardIndex||0)+1)+" / "+String(context.cardCount):"");set("headline",headline);set("text",text);set("source",source);',
