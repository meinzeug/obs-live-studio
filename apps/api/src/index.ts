...
function rendererHtml(dataUrl: string, overlayToken?: string) {
  ...
  const script = [
    ...
    'const HOST_VISUAL_LEAD_MS=1600,HOST_LAYER_STABLE_MS=550,HOST_DUCK_LEAD_MS=650,HOST_FRAME_TIMEOUT_MS=6000;',
    'let currentVersion=-1,currentDoc=null,activeHostAudio=null,activeHostAudioTurn=null,activeHostItemId=null,pendingHostAudioTurn=null,aiHostLayer=null,aiHostTurnId=null,youtubeContextStage=null,studioBrandBackground=null,lastHostState=null,lastYoutubeContextState=null;',
    ...
    '  const turn=host?.turn;const audioActive=Boolean(turn&&activeHostAudioTurn===turn.id),audioStarting=Boolean(turn&&pendingHostAudioTurn===turn.id&&revealedHostAudioTurns.has(turn.id)),speaking=audioActive||audioStarting,chatSpeaking=speaking&&isContextChatTurn(turn),preparingChat=Boolean(turn&&pendingHostAudioTurn===turn.id&&!revealedHostAudioTurns.has(turn.id)&&isContextChatTurn(turn));youtubeContextStage.dataset.turnId=turn?.id||"";youtubeContextStage.className="youtube-context-stage"+(speaking?" speaking":"")+(chatSpeaking?" chat-speaking":speaking?" ava-speaking":"")+(preparingChat?" preparing-chat":"");setContextAvatarVideo(youtubeContextStage,chatSpeaking||!speaking?(host?.moderator?.idleVideoUrl||context.idleVideoUrl):(host?.moderator?.speakingVideoUrl||context.speakingVideoUrl));setContextChatVideo(youtubeContextStage,host?.chatModerator?.videoUrl||host?.moderator?.chatModeratorVideoUrl||context.chatModeratorVideoUrl,chatSpeaking||preparingChat);',
