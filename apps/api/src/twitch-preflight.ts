...
const PATCH_MARKER = Symbol.for('argumentationskette.obs.twitch-preflight');
...
export function installTwitchStreamPreflight(options: InstallOptions = {}) {
  const prototype = ObsController.prototype as typeof ObsController.prototype & {
    [PATCH_MARKER]?: { original: typeof ObsController.prototype.startStream; wrapped: typeof ObsController.prototype.startStream };
  };
  if (prototype[PATCH_MARKER]) return () => undefined;

  const original = prototype.startStream;
  const wrapped: typeof original = async function (...args) {
    const env = options.environmentProvider?.() ?? process.env;
    const inspectOptions = options.inspectOptionsProvider?.() ?? {};
    await assertTwitchRuntimeReady(env, inspectOptions);
