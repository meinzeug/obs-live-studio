...
export async function inspectTwitchRuntime(
  ...
  try {
    ...
    add(
      'plugin-config-owner',
      configurationOwnedByProcess ? 'ok' : 'error',
      configurationOwnedByProcess
        ? 'Die Plugin-Konfiguration gehört dem laufenden Dienstbenutzer.'
        : 'Die Plugin-Konfiguration gehört einem anderen Benutzer.',
    );
  } catch (error: any) {
    add(
      'plugin-config',
