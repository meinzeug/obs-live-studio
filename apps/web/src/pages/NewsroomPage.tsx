...
export function NewsroomPage({ user }: { user: SessionUser }) {
  ...
  async function saveArticle() {
    ...
    try {
      ...
      setArticles((current) =>
        current.map((article) => (article.id === updated.id ? { ...article, ...updated } : article)),
      );
  ...
  const filtered = useMemo(() => {
    ...
    return articles.filter(
      (article) =>
        !normalized ||
        `${article.title} ${article.excerpt ?? ''} ${article.source_name ?? ''}`
    ...
    <section className="workspace-hub newsroom-page">
      <header className="workspace-page-header">
        <div>
          ...
          <h1>Newsroom</h1>
          <p>Von der Quelle bis zur freigegebenen Sendung – alle redaktionellen Schritte an einem Ort.</p>
        </div>
        <div className="workspace-header-actions">
          <button onClick={() => void load()} disabled={loading}>
            <RefreshCw size={17} className={loading ? 'spin' : ''} /> Aktualisieren
          </button>
          <button className="primary-button" disabled={!editable} onClick={() => setCreating(true)}>
            <Plus size={17} /> News erstellen
          </button>
          <Link className="button" to={`${routes.sources}?create=true`}>
            <Plus size={17} /> Quelle hinzufügen
          </Link>
        </div>
      </header>
      ...
      <div className="newsroom-kpis">
        <Link to={articlesRoute({ status: 'new' })}>
          <span className="hub-kpi-icon blue">
            <Newspaper />
          </span>
          <div>
            <small>Redaktionseingang</small>
            <strong>{dashboard?.counts.newArticles ?? 0}</strong>
            <span>neue Beiträge</span>
          </div>
          <ArrowRight />
        </Link>
        <Link to={articlesRoute({ warnings: true })}>
          <span className="hub-kpi-icon amber">
            <AlertTriangle />
          </span>
          <div>
            <small>Prüfung nötig</small>
            <strong>{warningArticles}</strong>
            <span>mit Warnhinweisen</span>
          </div>
          <ArrowRight />
        </Link>
        <Link to={articlesRoute({ status: 'approved' })}>
          <span className="hub-kpi-icon green">
            <FileCheck2 />
          </span>
          <div>
        ...
        <Link to={sourceHealthRoute({ state: 'problem' })}>
          <span className="hub-kpi-icon rose">
            <HeartPulse />
          </span>
          <div>
            <small>Quellenzustand</small>
            <strong>{health?.overview.down ?? dashboard?.counts.failedSources ?? 0}</strong>
            <span>Fehler in 24 Stunden</span>
          </div>
          <ArrowRight />
        </Link>
      </div>
      ...
      <div className="newsroom-toolbar">
        <div className="studio-search-field">
          ...
          <input
            ...
            placeholder="Beiträge, Quellen und Themen durchsuchen …"
          />
          <kbd>⌘ K</kbd>
        </div>
        <div className="newsroom-filter-links">
          <Link to={articlesRoute({ status: 'new' })}>Neu</Link>
          <Link to={articlesRoute({ warnings: true })}>Zu prüfen</Link>
          <Link to={articlesRoute({ status: 'approved' })}>Freigegeben</Link>
          <Link to={routes.articles}>Alle Beiträge</Link>
        </div>
      </div>
      ...
      <div className="newsroom-layout">
        <section className="hub-panel editorial-inbox">
          <header>
            <div>
              <p className="eyebrow">Aktueller Eingang</p>
              <h2>Neueste Meldungen</h2>
            </div>
            <span>{filtered.length} Beiträge</span>
          </header>
          <div className="editorial-list">
            {latest.map((article) => {
              ...
              return (
                <article className="editorial-row" key={article.id}>
                  <div className="editorial-time">
                    <strong>{date.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</strong>
                    <small>{date.toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' })}</small>
                  </div>
                  <div className="editorial-copy">
                    <span>
                      <em>{article.source_name ?? 'Unbekannte Quelle'}</em>
                      {warnings > 0 && (
                        <i className="warning">
                    ...
                    <p>{article.excerpt || 'Noch kein redaktioneller Auszug verfügbar.'}</p>
                  </div>
                  <div className="editorial-score">
                    <span>{article.trust_score ?? 0}%</span>
            ...
            {!loading && latest.length === 0 && (
              <div className="hub-empty">
                <Search size={22} />
                <strong>Keine passenden Beiträge</strong>
                <span>Passe die Suche an oder synchronisiere deine Quellen.</span>
              </div>
            )}
          </div>
          <footer>
            <Link to={routes.articles}>
              Alle Nachrichten öffnen <ArrowRight size={15} />
            </Link>
          </footer>
        </section>
        ...
        <aside className="newsroom-side-column">
          <section className="hub-panel source-pulse">
            ...
            <div className="source-health-ring-row">
              <div
                className="source-health-ring"
                style={
                  {
                    '--healthy': `${health?.overview.totalSources ? (health.overview.healthy / health.overview.totalSources) * 100 : 0}%`,
                  } as React.CSSProperties
                }
              >
                <span>
                  <strong>{health?.overview.healthy ?? 0}</strong>
                  <small>gesund</small>
                </span>
              </div>
              <dl>
                <div>
                  ...
                  <dd>{health?.overview.healthy ?? 0}</dd>
                </div>
                <div>
                  <dt>
                    <i className="warning" />
                  ...
                  <dd>{health?.overview.degraded ?? 0}</dd>
                </div>
                <div>
                  <dt>
                    <i className="error" />
            ...
            <div className="problem-sources">
              {health?.items
                ...
                .slice(0, 3)
                .map((item) => (
                  <Link key={item.sourceId} to={sourceHealthRoute({ source: item.sourceId })}>
                    <span>
                      <strong>{item.name}</strong>
                      <small>{item.consecutiveFailures} aufeinanderfolgende Fehler</small>
                    </span>
                    <AlertTriangle size={15} />
                  </Link>
                ))}
              {health && health.items.every((item) => item.state === 'healthy') && (
                <div className="all-clear">
                  <CheckCircle2 size={17} />
                  <span>Alle Quellen arbeiten normal.</span>
                </div>
              )}
            </div>
            <footer>
              <Link to={routes.sourceHealth}>
                Quellenmonitor öffnen <ArrowRight size={15} />
              </Link>
            </footer>
          </section>
          ...
          <section className="hub-panel newsroom-ai-card">
            <header>
              <div>
                <p className="eyebrow">Redaktionshilfe</p>
                <h2>KI Studio</h2>
              </div>
              <Sparkles size={19} />
            </header>
            ...
            <div>
              <span>
                <ShieldCheck size={14} /> Quellengetreu
              </span>
              <span>
                <FileCheck2 size={14} /> Redaktionell prüfbar
              </span>
            </div>
            <Link className="button" to={routes.aiStudio}>
              KI-Werkzeuge öffnen <ArrowRight size={15} />
            </Link>
          </section>
        </aside>
      </div>
      ...
      <section className="hub-panel newsroom-video-strip">
        <header>
          <div>
            <p className="eyebrow">YouTube-Redaktion</p>
            <h2>Neu in der Videothek</h2>
          </div>
          <Link to={routes.youtubeVideos}>
            Alle Videos <ArrowRight size={15} />
          </Link>
        </header>
        <div>
          {youtube.slice(0, 4).map((video) => (
            <Link key={video.id} to={routes.youtubeVideos}>
              <span className="video-strip-thumb">
                <PlayCircle size={25} />
              </span>
              <span>
                <small>{video.channel_name || 'YouTube'}</small>
                <strong>{video.title}</strong>
                <em>
                  <Clock3 size={12} /> {Math.max(1, Math.round((video.duration_seconds ?? 0) / 60))} Min.
                </em>
              </span>
          ...
          {youtube.length === 0 && (
            <div className="hub-empty">
              <Video size={22} />
              <strong>Noch keine YouTube-Videos</strong>
              <span>Füge einen Kanal oder ein einzelnes Video hinzu.</span>
            </div>
          )}
        </div>
      </section>
      {editingArticle && (
        <div className="studio-modal-backdrop">
          <div
            className="studio-dialog article-edit-dialog"
            role="dialog"
            aria-modal="true"
            aria-label="Nachricht bearbeiten"
          >
        ...
        <div className="studio-modal-backdrop">
          ...
          >
            ...
            <p className="dialog-note">
              Manuell erstellte News werden als „Review“ gespeichert und können danach wie importierte Beiträge
              freigegeben, mit Medien versehen und vertont werden.
            </p>
            <footer>
              ...
              <button
                ...
                disabled={Boolean(busy) || createForm.title.trim().length < 3}
