  ...
  return (
    <section className="workspace-hub newsroom-page">
      <header className="workspace-page-header">
        ...
        <div className="workspace-header-actions"><button onClick={() => void load()} disabled={loading}><RefreshCw size={17} className={loading ? 'spin' : ''} /> Aktualisieren</button><Link className="button primary-button" to={`${routes.sources}?create=true`}><Plus size={17} /> Quelle hinzufügen</Link></div>
      </header>
        ...
        <section className="hub-panel editorial-inbox">
          ...
          <div className="editorial-list">
            {latest.map((article) => {
              ...
              return <Link key={article.id} to={articleDetailRoute(article.id)}>
                ...
                <div className="editorial-score"><span>{article.trust_score ?? 0}%</span><small>Vertrauen</small><ArrowRight size={15} /></div>
              </Link>;
            })}
