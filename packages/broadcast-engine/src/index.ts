...
export class BroadcastRunner {
  ...
  private take(c: Control) {
    const i = this.controls.indexOf(c);
    if (i < 0) return false;
    this.controls.splice(i, 1);
    return true;
  }
  private has(c: Control) {
    return this.controls.includes(c);
  }
  async start() {
    ...
    try {
      ...
      await setPlaybackState({ status: 'idle' });
    } catch (e) {
      if (e instanceof ControlledStop) {
        ...
        await setPlaybackState({ status: e.finalStatus });
        return;
      }
      ...
      await setPlaybackState({ status: 'error', error });
      throw e;
    } finally {
      this.running = false;
    }
  }
  private async waitWhilePaused(runId: string, item?: { id: string; article_id: string }, position?: number) {
    if (!this.take('pause') && !this.has('pause')) return;
    await setBroadcastPlaylistState(this.opts.playlistId, 'paused');
    await updateBroadcastRun(runId, 'paused', {
      status: 'paused',
      itemId: item?.id,
    ...
    while (!this.take('resume')) {
      if (this.take('stop')) throw new ControlledStop('interrupted');
      await new Promise((r) => setTimeout(r, this.opts.pollMs ?? 100));
    }
    await updateBroadcastRun(runId, 'running', {
      status: 'running',
      itemId: item?.id,
      articleId: item?.article_id,
      position,
  ...
  private async loop(runId: string) {
    ...
    for (let i = playlist.current_position; i < items.length; i++) {
      ...
      await this.waitWhilePaused(runId, item, i);
      if (this.take('stop')) break;
      if (this.take('skip')) {
      ...
      try {
        ...
        await setPlaybackState({ status: 'preparing', ...base });
        ...
        await this.opts.obs.playTestContribution({
          ...
          onState: async (s) => {
            await updateBroadcastRun(runId, s.status === 'paused' ? 'paused' : 'running', { ...s, ...base });
            await setPlaybackState({ ...s, ...base });
          },
          control: async () => {
            if (this.take('stop')) return 'stop';
            if (this.take('skip')) return 'skip';
            if (this.take('pause')) return 'pause';
            return undefined;
          },
          onPaused: () => this.waitWhilePaused(runId, item, i),
        });
