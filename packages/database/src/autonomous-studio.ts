  ...
  const updated = (
    await query<AutonomousStudioSettings>(
      `update autonomous_studio_settings set
         ...
         audience_council_max_daily=coalesce($18,audience_council_max_daily),updated_at=now()
  ...
  const metrics = (
    await query<{
      ...
      `select count(*)::int total,
       ...
       count(*) filter(where decision.status in ('awaiting_council','awaiting_reviews','approved','applying'))::int under_review,
...
export async function createAutonomousStudioDecision(input: {
  ...
  const inserted = (
    await query<AutonomousStudioDecision>(
      `insert into autonomous_studio_decisions(
         parent_decision_id,kind,source,title,instruction,requested_by,requested_by_system,
         proposal,proposal_model,proposal_usage,status
...
export async function getAutonomousStudioDecision(id: string) {
  ...
  const [councilVotes, reviews, events] = await Promise.all([
  ...
  return { ...decision, councilVotes: councilVotes.rows, reviews: reviews.rows, events: events.rows };
}
...
export async function queueAutonomousStudioCycle(
  ...
  return transaction(async (client) => {
    ...
    const existing = (
      await client.query<AutonomousStudioDecision>(
        `select * from autonomous_studio_decisions
         where kind='strategy' and status in ('queued','planning','awaiting_council','awaiting_reviews','approved','applying')
    ...
    const decision = (
      await client.query<AutonomousStudioDecision>(
        `insert into autonomous_studio_decisions(kind,source,title,instruction,requested_by,requested_by_system)
         values('strategy',$1,'Autonomer Senderausbau',$2,$3,$4) returning *`,
        [
