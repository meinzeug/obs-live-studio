  ...
  for (let redirects = 0; redirects <= maxRedirects; redirects++) {
    ...
    try {
      ...
      if ([301, 302, 303, 307, 308].includes(res.status)) {
        ...
        if (!location) throw new Error(`Redirect ohne Location von ${current}`);
      ...
      if ((e as Error).name === 'AbortError') throw new Error(`Timeout nach ${timeoutMs} ms beim Abruf von ${current}`);
      throw e;
    } finally {
      clearTimeout(timer);
    }
