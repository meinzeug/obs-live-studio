...
describe('article to broadcast milestone',()=>{
  it('extracts article text without executable markup',()=>{
    const parsed=parseHtmlArticle('<main><h1>Hallo</h1><script>alert(1)</script><style>body{}</style><p>Dies ist der sichere Haupttext eines Artikels mit genügend Inhalt für einen Beitrag.</p></main>','https://example.org/a');
  ...
  it('creates summary and speaker script from stored article text',()=>{
    const text='Die Redaktion prüft einen neuen Nachrichtenartikel. Der Text enthält belegte Informationen und wird für den Sendebeitrag verdichtet. Weitere Details bleiben im Originalartikel nachvollziehbar.';
