...
describe('shared broadcast operations workflow', () => {
  ...
  it('connects planning and control room through one status model and explicit return strategies', async () => {
    ...
    expect(navigation).toContain("id: 'liveStudio'");
    expect(navigation).toContain("label: 'Live-Studio'");
    expect(navigation).toContain('matches: [routes.broadcast]');
    expect(navigation).toContain('matches: [routes.live]');
