...
import { auditReadmeContracts } from '../scripts/readme-contracts-v2.mjs';
...
describe('README implementation contracts', () => {
  it('matches every advertised core capability to its implementation', async () => {
    ...
    expect(report.contracts).toBe(41);
    expect(report.checks).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: 'mandatory-article-visual-media', ok: true })]),
    );
  });
  ...
  it('fails closed when advertised implementation files are absent', async () => {
    ...
    expect(report.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: 'mandatory-article-visual-media', ok: false }),
