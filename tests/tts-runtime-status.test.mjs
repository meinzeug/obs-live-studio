...
async function createPiperRuntime(root, options = {}) {
  ...
  await writeFile(
    `${model}.json`,
    options.config ?? JSON.stringify({ language: { code: 'de_DE' }, audio: { sample_rate: 22_050 }, num_speakers: 1 }),
  );
...
describe('TTS runtime health', () => {
  it('resolves Piper with Thorsten High as the default runtime', () => {
    const runtime = resolveTtsRuntime({}, '/srv/studio');
  ...
  it('accepts a complete Thorsten High installation', async () => {
    ...
    expect(report.model).toEqual(expect.objectContaining({ language: 'de_DE', sampleRate: 22_050, speakers: 1 }));
  });
  ...
  it('rejects invalid model metadata and unsupported engines', async () => {
    ...
    const unsupported = await inspectTtsRuntime({ root, env: { TTS_ENGINE: 'cloud-voice' } });
  ...
  it('replaces the old shallow TTS preflight result and recomputes the summary', async () => {
    ...
    expect(report.tts).toEqual({ ok: false, engine: 'piper', voice: 'de_DE-thorsten-high', model: null });
  });
});
